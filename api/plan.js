...
- Fractions — naming, equivalence, comparison (unlocks after 50 fluent facts); once two of those three are solid, adding and subtracting like fractions and reading fractions on a number line (0 to 1, sometimes 0 to 2). Typical errors: adding the bottom numbers too (3/8 + 2/8 = 5/16), and counting tick marks instead of spaces on the number line.
...
- Two-step problems (unlocks once 3 of the 4 word-problem schemas are solid): groups then more or less, compare then total, two changes in a row, and — once long division is open — leftovers, where the question decides whether a remainder means one more group, is dropped, or is the answer. Typical errors: stopping after the first step, running one operation over every number, and mishandling the leftover.
